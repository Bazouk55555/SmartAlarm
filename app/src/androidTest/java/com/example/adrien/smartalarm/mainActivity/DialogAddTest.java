...
public class DialogAddTest {
	...
	public void toneTest() {
		...
		onData(anything()).inAdapterView(withId(R.id.list_tone)).atPosition(1).perform(click());
		onData(anything()).inAdapterView(withId(R.id.list_tone))
				.check(matches(withText(mActivityRule.getActivity().getResources().getString(R.string.alarm2))));
		onView(withId(R.id.cancel)).perform(click());
	}
}
