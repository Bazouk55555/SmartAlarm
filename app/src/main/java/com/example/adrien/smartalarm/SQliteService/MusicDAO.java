...
public class MusicDAO extends AbstractBaseDAO {
    ...
    public MusicDAO(Context pContext) {
        this.databaseHandler = new DatabaseMusicHandler(pContext, DATABASE, null, VERSION);
    }
    ...
    public void add(Question question) {
        ...
        mDb.insert(((DatabaseMusicHandler)databaseHandler).TABLE_NAME, null, value);
    }

    public Question select() {
        Cursor c = mDb.rawQuery("select * from " + ((DatabaseMusicHandler)databaseHandler).TABLE_NAME + " LIMIT ?", new String[]{"1"});
        int idKey=0;
        String question="";
        String answer="";
        String wrongAnswer1="";
