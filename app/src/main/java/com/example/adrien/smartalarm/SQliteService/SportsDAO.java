...
public class SportsDAO extends AbstractQuestionBaseDAO {
	...
	public int getNumberOfQuestionInFile() {
		return mDb.rawQuery("select * from " + DatabaseQuestionHandler.TABLE_SPORTS_NAME, null).getCount();
	}

	@Override
	public List<Question> select(int numberOfQuestion) {
		String query = "select * from " + DatabaseQuestionHandler.TABLE_SPORTS_NAME;
